...
async function main() {
    ...
    for (const event of EVENTS) {
        const exists = await prisma.event.findFirst({ where: { title: (event.title as LocalizedString).id } });
        if (!exists) {
            await prisma.event.create({
                data: {
                    title: (event.title as LocalizedString).id,
                    location: event.location,
                    date: event.date,
                    description: (event.description as LocalizedString).id,
    ...
    for (const pkg of PACKAGES) {
        const exists = await prisma.tourPackage.findFirst({ where: { title: (pkg.title as LocalizedString).id } });
        if (!exists) {
            await prisma.tourPackage.create({
                data: {
                    title: (pkg.title as LocalizedString).id,
                    duration: pkg.duration,
                    price: pkg.price,
                    location: pkg.location,
                    rating: pkg.rating,
                    ecoRating: pkg.ecoRating,
                    description: (pkg.description as LocalizedString).id,
