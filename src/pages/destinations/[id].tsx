...
export default function DestinationDetail({ region, relatedPackages }: DestinationDetailProps) {
    const { scrollY } = useScroll();
    const { addToast } = useToast();

    // Fallback if region is null (though SSR handles redirect usually)
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const saved = localStorage.getItem(`wishlist_destination_${region.id}`);
    ...
    const handleLike = () => {
        if (isLiked) {
            localStorage.removeItem(`wishlist_destination_${region.id}`);
            setIsLiked(false);
            addToast('Dihapus dari wishlist', 'info');
        } else {
            localStorage.setItem(`wishlist_destination_${region.id}`, 'true');
            setIsLiked(true);
        ...
        <>
            <Layout title={`${region.name} - BorneoTrip Destinasi`}>
                <div className="bg-slate-50 min-h-screen pb-20">
                    ...
                    <div className="relative h-[85vh] md:h-[95vh] w-full overflow-hidden bg-slate-900">
                        ...
                        <motion.div
                            ...
                            className="absolute bottom-0 left-0 w-full p-6 md:p-12 lg:p-24 pb-32 md:pb-64 z-10 flex flex-col justify-end h-full"
                        >
                            <div className="max-w-7xl mx-auto w-full mb-4 md:mb-10">
                                <motion.div
                                ...
                                >
                                    <div className="flex flex-wrap items-center gap-3 mb-4 md:mb-6">
                                        <span className={`inline-block px-3 py-1 md:px-4 md:py-1.5 rounded-full text-[10px] md:text-sm font-bold uppercase tracking-widest backdrop-blur-md border border-white/20 text-white bg-white/10 shadow-lg`}>
                                            Explore {region.type}
                                        </span>
                                        <span className="flex items-center gap-1 px-3 py-1 md:py-1.5 rounded-full bg-emerald-500 text-white text-[10px] md:text-xs font-bold uppercase tracking-wider shadow-lg shadow-emerald-500/20">
                                            <Camera className="w-3 h-3" /> Top Rated
                                        </span>
                                    </div>

                                    <h1 className="text-5xl md:text-8xl lg:text-9xl font-black text-white mb-4 md:mb-6 tracking-tighter leading-none drop-shadow-2xl">
                                        {region.name}
                                    </h1>

                                    <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-8 text-base md:text-lg text-gray-200 font-light">
                    ...
                    <div className="relative z-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto -mt-20 md:-mt-32">

                        {/* Floating Glass Stats Grid - Responsive Grid */}
                        <motion.div
                        ...
                        >
                            {[
                                ...
                                <div key={idx} className={`relative overflow-hidden bg-white/10 backdrop-blur-xl border border-white/20 p-4 md:p-6 rounded-2xl md:rounded-3xl shadow-2xl flex flex-col justify-between group h-28 md:h-40 hover:-translate-y-2 transition duration-500`}>
                                    ...
                                    <div className="relative z-10 flex justify-between items-start">
                                        ...
                                        <span className="text-[10px] md:text-xs font-bold text-gray-300 md:text-gray-400 uppercase tracking-wider bg-black/40 md:bg-black/20 px-2 py-1 rounded-lg">{stat.unit}</span>
                                    </div>
                                    <div className="relative z-10 mt-auto">
                                        <div className="text-xl md:text-4xl font-black text-white drop-shadow-md truncate">{stat.value}</div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 pb-20">
                            ...
                            <div className="lg:col-span-8 space-y-8 md:space-y-12">
                                ...
                                {/* TABS - Desktop only? Or scrollable on mobile */}
                                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
                                    {['Overview', 'Destinations', 'Packages'].map((tab) => (
                                        <button
                                            ...
                                            className="px-5 py-2.5 md:px-6 md:py-3 rounded-full bg-white border border-gray-100 text-gray-600 font-bold text-sm hover:bg-emerald-50 hover:text-emerald-700 hover:border-emerald-200 transition shadow-sm whitespace-nowrap"
                                        >
                                            {tab}
                                        </button>
                                    ))}
                                ...
                                <section id="overview" className="scroll-mt-32">
                                    <motion.div
                                        ...
                                        whileInView={{ opacity: 1, y: 0 }}
                                        viewport={{ once: true }}
                                        className="prose prose-lg md:prose-xl prose-slate max-w-none bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100"
                                    >
                                        <h3 className="text-emerald-600 font-bold uppercase tracking-widest text-xs md:text-sm mb-2">The Jewel of East Kalimantan</h3>
                                        ...
                                        <p className="text-slate-600 leading-loose text-base md:text-lg">
                                            <span className="text-5xl md:text-6xl float-left mr-3 mt-[-10px] font-black text-emerald-500 font-serif">"{region.name.charAt(0)}</span>
                                        ...
                                        <p className="text-slate-600 leading-loose mt-4 md:mt-6 text-base md:text-lg">
                                ...
                                <section id="destinations" className="scroll-mt-32">
                                    <div className="flex items-end justify-between mb-6 md:mb-8">
                                        <div>
                                            <h3 className="text-emerald-600 font-bold uppercase tracking-widest text-xs md:text-sm mb-2">Must Visit Places</h3>
                                            <h2 className="text-2xl md:text-4xl font-black text-slate-900">Destinasi Ikonik</h2>
                                        </div>
                                        ...
                                        {relatedPackages.slice(0, 4).map((pkg, idx) => (
                                            <Link href={`/packages/${pkg.id}`} key={pkg.id} className={`group relative rounded-[2rem] overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl transition-all duration-500 ${idx === 0 ? 'md:col-span-2 aspect-2/1' : 'aspect-4/3'}`}>
                                                ...
                                                <div className="absolute top-4 right-4 md:top-6 md:right-6">
                                                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition transform translate-y-4 group-hover:translate-y-0">
                                                        <ArrowRight className="w-4 h-4 md:w-5 md:h-5 -rotate-45" />
                                                    </div>
                                                </div>

                                                <div className="absolute bottom-0 left-0 p-6 md:p-8">
                                                    <span className="inline-block px-3 py-1 rounded-lg bg-emerald-500 text-white text-[10px] font-bold uppercase tracking-wider mb-2 md:mb-3 shadow-lg">
                                                        Package
                                                    </span>
                                                    <h3 className={`font-black text-white leading-tight mb-2 ${idx === 0 ? 'text-2xl md:text-4xl' : 'text-xl md:text-2xl'}`}>
                                ...
                                <section id="packages" className="scroll-mt-32">
                                    <h3 className="text-emerald-600 font-bold uppercase tracking-widest text-xs md:text-sm mb-2">Curated Trips</h3>
                                    <h2 className="text-2xl md:text-4xl font-black text-slate-900 mb-6 md:mb-8">Paket Wisata Terkait</h2>
                            ...
                            <div className="lg:col-span-4 relative">
                                <div className="static lg:sticky lg:top-28 space-y-6">
                                    ...
                                    {/* Booking Card */}
                                    <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 p-6 md:p-8 relative overflow-hidden">
                                        ...
                                        <p className="text-sm text-gray-500 mb-6 md:mb-8 relative z-10">Jelajahi paket wisata terbaik yang tersedia di {region.name}.</p>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    ...
    // Parse ID (assuming it's a number based on prisma schema but URL is string)
    ...
    try {
        ...
        // Find relevant packages string matching
        ...
        // Serialization for JSON (Dates need to be strings)
        ...
        return {
            props: {
                ...
                relatedPackages: serializedPackages
            }
        };
    } catch (error) {
