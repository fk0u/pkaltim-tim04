...
import { Clock, MapPin, Search, ArrowRight, ListFilter, Leaf } from 'lucide-react';
  ...
  return (
    <Layout title="Paket Wisata - BorneoTrip">
      <div className="bg-emerald-950 text-white py-20 relative overflow-hidden">
         <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1518182170546-0766ce6fdd93?auto=format&fit=crop&q=80')] bg-cover bg-center opacity-40"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Filter Bar */}
        <div className="sticky top-20 z-30 bg-white/80 backdrop-blur-md p-2 rounded-2xl shadow-lg border border-gray-200 mb-10 flex flex-col md:flex-row gap-4 items-center justify-between transition-all">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredPackages.map((pkg) => (
            <Link href={`/packages/${pkg.id}`} key={pkg.id} className="group h-full">
