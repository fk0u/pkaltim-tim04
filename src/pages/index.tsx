   ...
   return (
      ...
      >
                  ...
                  {popularDestinations.map((dest, i) => (
                     <Link href={`/destinations/${dest.id}`} key={dest.id} className="group relative aspect-4/5 rounded-3xl overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                        ...
                        <div className="absolute bottom-0 left-0 p-6 w-full">
                           <h3 className="text-xl font-bold text-white mb-1 group-hover:text-emerald-300 transition">{dest.name}</h3>
                           <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold uppercase tracking-wider mb-2">
                              <MapPin className="w-3 h-3" /> {dest.type}
                           </div>
                           <p className="text-gray-300 text-xs line-clamp-2 opacity-0 group-hover:opacity-100 transition transform translate-y-4 group-hover:translate-y-0 duration-500 delay-100">
                              Explore {dest.name} and its wonders.
                           </p>
                        </div>
                     </Link>
                  ))}
         ...
         <section className="py-24 bg-white relative overflow-hidden">
            <div className="absolute top-0 right-0 w-1/3 h-full bg-emerald-50 skew-x-12 translate-x-32"></div>
            <motion.div {...fadeInUp} className="max-w-7xl mx-auto px-4 relative z-10 text-center">
               <span className="inline-block py-1 px-3 rounded-full bg-emerald-100 text-emerald-700 text-xs font-bold tracking-widest uppercase mb-6">Sustainable Tourism</span>
               <h2 className="text-4xl md:text-5xl font-black text-gray-900 mb-6">{t.homepage.sustainabilityTitle} {t.homepage.sustainabilityTitleHighlight}</h2>
               <p className="text-xl text-gray-500 max-w-3xl mx-auto mb-16 leading-relaxed">
                  {t.homepage.sustainabilityDesc}
               </p>

               <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
                  <div className="p-8 rounded-3xl bg-white border border-gray-100 shadow-xl hover:shadow-2xl transition duration-500 group">
                     <div className="w-16 h-16 rounded-2xl bg-emerald-100 flex items-center justify-center text-4xl mb-6 group-hover:scale-110 transition">🌱</div>
                     <div className="text-3xl font-bold text-gray-900 mb-1">500+</div>
                     <div className="text-sm font-bold text-gray-400 uppercase tracking-widest">Trees Planted</div>
                  </div>
                  <div className="p-8 rounded-3xl bg-white border border-gray-100 shadow-xl hover:shadow-2xl transition duration-500 group">
                     <div className="w-16 h-16 rounded-2xl bg-blue-100 flex items-center justify-center text-4xl mb-6 group-hover:scale-110 transition">💧</div>
                     <div className="text-3xl font-bold text-gray-900 mb-1">Zero</div>
                     <div className="text-sm font-bold text-gray-400 uppercase tracking-widest">Plastic Waste</div>
                  </div>
                  <div className="p-8 rounded-3xl bg-white border border-gray-100 shadow-xl hover:shadow-2xl transition duration-500 group">
                     <div className="w-16 h-16 rounded-2xl bg-amber-100 flex items-center justify-center text-4xl mb-6 group-hover:scale-110 transition">🤝</div>
                     <div className="text-3xl font-bold text-gray-900 mb-1">Rp 2M</div>
                     <div className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t.homepage.donationDistributed}</div>
                  </div>
               </div>

               <Link href="/about" className="group relative inline-flex items-center justify-center px-8 py-4 text-lg font-bold text-white transition-all duration-200 bg-gray-900 font-pj rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 hover:bg-emerald-600">
