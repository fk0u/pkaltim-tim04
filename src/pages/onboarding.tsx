            ...
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center pt-20 pb-12 px-4">
                ...
                <div className="w-full max-w-xl mb-8">
                    <div className="flex justify-between text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
                        ...
                        <span className={step >= 4 ? 'text-emerald-600' : ''}>Profil</span>
                    </div>
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {step === 4 && (
                            <motion.div
                                ...
                                className="flex-1"
                            >
                                <h2 className="text-2xl font-bold text-slate-900 mb-2">Lengkapi Data Diri</h2>
                                <p className="text-slate-500 mb-6">Agar kami bisa mengenalmu lebih baik dan memudahkan pemesanan.</p>

                                <div className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-1">Nomor Telepon / WhatsApp</label>
                                        <input
                                            type="text"
                                            placeholder="08123456789"
                                            value={preferences.phone}
                                            onChange={(e) => setPreferences(p => ({ ...p, phone: e.target.value }))}
                                            className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-1">Nomor KTP / Identitas</label>
                                        <input
                                            type="text"
                                            placeholder="16 digit NIK"
                                            value={preferences.idNumber}
                                            onChange={(e) => setPreferences(p => ({ ...p, idNumber: e.target.value }))}
                                            className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition"
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-1">Bio Singkat (Opsional)</label>
                                        <textarea
                                            placeholder="Ceritakan sedikit tentang dirimu..."
                                            value={preferences.bio}
                                            onChange={(e) => setPreferences(p => ({ ...p, bio: e.target.value }))}
                                            className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition h-24 resize-none"
                                        />
                                    </div>
                                </div>
                            </motion.div>
                    ...
                    <div className="mt-8 flex justify-between items-center pt-6 border-t border-slate-100">
                        ...
                        <button
                            ...
                            disabled={
                                ...
                                (step === 4 && (!preferences.phone || !preferences.idNumber))
                            }
                            className={`
                    ...
                    `}
                        >
                            {loading ? (
                                ...
                                <>
                                    {step === 4 ? 'Selesai' : 'Lanjut'} <ArrowRight size={20} />
                                </>
                            )}
                        </button>
                    </div>
